...
for(i in 1:2){
  ...
  # 
  # size = c(1.8, 2.5, 2.5)
  # cutline = c(0.011, 0.013, 0.013)
  # df1$labels$Group <- "B"
  # ptree = ggplot(segment(df1)) +
  #   geom_segment(aes(
  #     x = x,
  #     y = y,
  #     xend = xend,
  ...
  #   theme_bw() + labs(x = "", y = "") +
  #   geom_point(
  #     data = filter(df1$labels, label == scity[1] |
  #                     label == scity[2]),
  #     aes(
  #       x = x,
  #       y = y,
  ...
  # ptree
  # 
  # ggsave(paste0("ptree", ".pdf"), height = 6, width = 12)
...
wzdat = filter(us, ctname != "Chongqing", ctname != "Shanghai") %>% merge(wzcpa, by =
                                                                            "ctname")
shdat = filter(us, ctname != "Chongqing", ctname != "Wenzhou") %>% merge(shcpa, by =
                                                                           "ctname")
